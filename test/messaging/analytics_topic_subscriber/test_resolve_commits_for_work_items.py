...
class TestResolveCommitsForWorkItems:
    ...
    def it_publishes_responses_correctly(self, work_items_commits_fixture):
        ...
        message = fake_send(
            ResolveCommitsForWorkItems(
                send=dict(
                    ...
                    new_work_items=[dict(**work_item, parent_key=uuid.uuid4().hex) for work_item in new_work_items]
                )
            )
        )
        ...
        publisher.assert_topic_called_with_message(AnalyticsTopic, WorkItemsCommitsResolved, call=0)
