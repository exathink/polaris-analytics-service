...
class TestWorkItemImplementationEffort:
    ...
    def it_allocates_fractional_days_when_the_author_has_untracked_commits_on_a_coding_day(
            self,
        ...
        # Effort should be 0.5 since the author has one untracked commit on the day the work item was committed to
        assert db.connection().execute(
            f"select effort from analytics.work_items where key='{test_work_item['key']}'"
        ).scalar() == 0.5
