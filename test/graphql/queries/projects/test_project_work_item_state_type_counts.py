...
class TestProjectSpecStateTypeCounts:
    ...
    def it_returns_spec_counts_when_there_are_no_specs_in_the_project(self,
        ...
        result = client.execute(query, variable_values=dict(project_key=project.key))
        assert 'data' in result
        state_type_counts = result['data']['project']['specStateTypeCounts']
        assert state_type_counts['backlog'] == 0
        assert state_type_counts['open'] == 0
        assert state_type_counts['wip'] == 0
        assert state_type_counts['closed'] == 0
        assert state_type_counts['complete'] is None


    def it_returns_spec_counts_when_there_are_specs_in_the_project(self,
                                                                                     api_work_items_import_fixture):
        organization, project, work_items_source, work_items_common = api_work_items_import_fixture
    ...
    def it_respect_the_defects_only_parameter(self,
        ...
        work_items[3]['is_bug'] = False
        work_items[5]['is_bug'] = False
    ...
    def it_respect_the_closed_within_days_parameter(self,
        ...
        api_helper.update_delivery_cycles(
            [
                (3, dict(property='commit_count', value=2)),
                (3, dict(property='end_date', value=end_date)),
                (5, dict(property='commit_count', value=1))
            ])
