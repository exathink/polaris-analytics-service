...
from polaris.analytics.db.model import teams, contributors_teams, \
    ...
    teams_repositories, work_items_pull_requests, organizations, work_item_state_transitions
...
class TeamArrivalDepartureTrends(InterfaceResolver):
    ...
    def interface_selector(team_nodes, **kwargs):
        ...
        if arrival_departure_trends_args is None:
            raise ProcessingException(
                'arrivalDepartureTrendsArgs parameter must be specified to resolve ArrivalDepartureTrendsInterface')
        ...
        team_nodes_dates = select([team_nodes, timeline_dates]).cte('team_nodes_dates')
        ...
        select_arrivals = work_item_delivery_cycles_connection_apply_filters(select_arrivals, work_items,
                                                                             work_item_delivery_cycles,
                                                                             **arrival_departure_trends_args).alias(
            'select_arrivals')
        ...
        select_arrival_rate_trends = select([
            ...
            team_nodes_dates.outerjoin(
                select_arrivals,
                and_(
                    select_arrivals.c.team_id == team_nodes_dates.c.id,
                    select_arrivals.c.measurement_date == team_nodes_dates.c.measurement_date
                )
            )
        ).group_by(
            team_nodes_dates.c.id,
